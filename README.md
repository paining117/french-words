...
当前发布的 v1.0.0 安装包尚不包含下述 2026-09-24 界面、同义词提示及单词发音调整；这些调整可通过最新源码 / Expo Go 验收，独立安装包需重新构建。
...
- 四选一答对标绿，等待用户点「继续」才展示详情，不再自动跳转。答错时错误选项标红、正确选项标绿，法语紧随对应中文显示，点击继续查看详情。
...
- 单词详情自动播放一次系统法语发音，点击「发音」可重听；查词与学习/复习答案页共用，题目和拼写阶段不播放。离开详情、切词或进入后台会停止。优先法国法语，保留原词重音和连字，不朗读中文。
- 单词详情仅展示单词、用法、释义和例句等学习信息；来源与例句署名集中在「关于」。名词的 le/la 仅在详情中显示。词书浏览、切换、我的生词本与我的熟词本。

2026-09-24：统一隐藏导航栏页面名称，左上角仅保留返回。四选一排除全部释义有重叠的干扰词；拼写题对照整个本地词库检查同义词，bonjour / salut 补充问候场景区别，其余歧义使用可区分的开头字母和字母数提示。原始释义与学习数据不变。
...
运行时不使用远程词典、AI、账号、服务器或 API Key。单词发音使用 `expo-speech` 调用手机系统语音，不接外部音频服务；离线可用性取决于手机已安装的法语语音和系统引擎，需真机飞行模式验收。iPhone 请关闭静音并调高媒体音量（[Expo 说明](https://docs.expo.dev/versions/v57.0.0/sdk/speech/)）。首次启动导入内置词库，后续幂等检查。SQLite 自动迁移至 v6，保留原有词条、Cards、日志、签到、轮次和偏好；不要通过清除 App 数据来升级。
